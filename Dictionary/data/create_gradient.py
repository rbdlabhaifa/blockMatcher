...
# ARTHUR'S SCRIPT FOR CREATING GRADIENT IMAGES. FIXME: THERE IS A BIT OF NOISE IN IMAGES GENERATED BY THIS CODE.
...
def main_arthur_gradient():
    ...
    cam_matrix = np.array([[im_size[0] / (2 * np.tan(np.deg2rad(0.5*fov_x))), 0, im_size[0] / 2],
                           [0, im_size[1] / (2 * np.tan(np.deg2rad(0.5*fov_y))), im_size[1] / 2],
                           [0, 0, 1]])
