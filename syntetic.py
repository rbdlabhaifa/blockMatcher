...
def mapImage(im, T, sizeOutIm):
    ...
    source_coords = np.matmul(np.matmul(calib,np.matmul(np.linalg.inv(T),np.linalg.inv(calib))), source_coords)  # apply the transform
...
if __name__ == "__main__":
    ...
    vid = [BMFrame(im) for im in img_array]
    for i in range(26, len(vid)):
        frame1, frame2 = vid[i - 1], vid[i]
        mvs = BlockMatching.get_motion_vectors(frame2.base_image, frame1.base_image)
        frame1.draw_motion_vector(mvs, color=(0, 255, 0), thickness=2)
        frame1.show()
